...
This document describes the architecture of Flywall's integration test framework, which orchestrates parallel test execution across multiple QEMU virtual machines.
...
│  │                                                                        │  │
│  │  • Connects to /dev/virtio-ports/flywall.agent                          │  │
│  │  • Receives TEST commands from orca                                    │  │
│  │  • Executes test scripts, streams TAP output                           │  │
│  │  • Project mounted at /mnt/flywall (virtfs)                             │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
| `ProjectRoot`   | Mounted as virtfs share at `/mnt/flywall`         |
...
2. Starts QEMU with unique socket path (`/tmp/flywall-vm{id}.sock`)
...
args := []string{
    ...
    "-chardev", "socket,path=build/flywall-vm1.sock,server=on,wait=off",
...
1. Detects virtio-serial port (`/dev/virtio-ports/flywall.agent` or `/dev/vport0p1`)
...
#   build/flywall-orca       → symlink to toolbox
...
| Target | Description |
|--------|-------------|
| `fw build toolbox` | Build host + guest toolbox binaries |
| `fw test int` | Run parallel integration tests |
...
4. NewPod(cfg, 2).Start() → spawns 2 QEMU VMs
                    │
5. Workers wait for agent handshake on /tmp/flywall-vm{1,2}.sock
                    │
...
8. Worker sends: TEST t/01-sanity/sanity_test.sh\n
                    │
9. Agent executes: /bin/sh /mnt/flywall/t/01-sanity/sanity_test.sh
                    │
