...
# Cleans up Flywall processes, firewall rules, and network namespaces.
...
BRAND_LOWER="${BRAND_LOWER:-flywall}"
BINARY_NAME="${BINARY_NAME:-flywall}"
STATE_DIR="${STATE_DIR:-/opt/flywall/var/lib}"
...
# Avoid killing the agent itself (which might be named flywall-agent)
...
# 3. Wipe state directories (both default and test-env override)
rm -rf "$STATE_DIR"/* 2>/dev/null
# Also clean FLYWALL_STATE_DIR if set (tests use /var/lib/flywall)
if [ -n "${FLYWALL_STATE_DIR:-}" ] && [ "$FLYWALL_STATE_DIR" != "$STATE_DIR" ]; then
    rm -rf "$FLYWALL_STATE_DIR"/* 2>/dev/null
fi
# Explicitly remove crash state from common test location
rm -f /var/lib/flywall/crash.state 2>/dev/null
