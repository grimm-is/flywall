...
# Default to dev_mode=true if no kernel args provided (ensures flywall starts on make vm-start)
...
$QEMU_BIN \
    ...
    -chardev socket,path=${ORCHESTRATION_SOCKET:-/tmp/flywall-vm${VM_ID:-1}.sock},server=on,wait=off,id=channel0 \
