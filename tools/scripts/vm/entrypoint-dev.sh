...
if [ -f "$MOUNT_PATH/build/flywall-linux-$GOARCH" ]; then
    FIREWALL_BIN="$MOUNT_PATH/build/flywall-linux-$GOARCH"
else
    FIREWALL_BIN="$MOUNT_PATH/build/flywall"
fi
...
# Use /var/run for performance (tmpfs) instead of /opt (disk)
export FLYWALL_RUN_DIR="/var/run/flywall"
mkdir -p "$FLYWALL_RUN_DIR"
...
echo ""
echo "🌐 Starting API server (as unprivileged user)..."

# Start API server (unprivileged) in background
# Note: -user flag drops privileges after binding to port (prefers flywall, falls back to nobody)
...
if kill -0 $CTL_PID 2>/dev/null; then
    ...
    echo "  🔌 RPC:    /var/run/flywall/ctl.sock"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
