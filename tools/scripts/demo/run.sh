...
start_firewall() {
    ...
    "$SCRIPT_DIR/../vm/dev.sh" "$(pwd)" "" "dev_mode=true api_port=${API_PORT} config_file=/mnt/flywall/build/firewall.hcl" > /tmp/firewall-demo.log 2>&1 &
    local pid=$!
        ...
        dev)
            ...
            if [ -f "$FIREWALL_PID_FILE" ]; then
                ...
                if kill -0 "$PID" 2>/dev/null; then
                    ...
                    if [ ! -f "$BUILD_DIR/flywall" ]; then
                         print_error "Binary not found in build/flywall"
                         exit 1
                    fi
                    ...
                    ssh $SSH_OPTS root@localhost "/mnt/flywall/build/flywall upgrade /mnt/flywall/build/flywall"
                     ...
                     if [ $? -eq 0 ]; then
                        ...
                        exit 0
                         ...
                         print_warning "If SSH is not configured in the running VM, you must stop it first: fw vm stop"
                         exit 1
                    fi
                fi
            fi
