...
func TestManager_ApplyConfig_Integration(t *testing.T) {
	...
	if !foundInput {
		t.Error("Chain input not found in flywall table")
	}
	if !foundForward {
		t.Error("Chain forward not found in flywall table")
	}
	...
	var flywallTable *nftables.Table
	...
	if flywallTable == nil {
		t.Fatal("flywall table not found")
	}

	// Verify rules exist in forward chain
	rules, err := conn.GetRules(flywallTable, &nftables.Chain{Name: "forward", Table: flywallTable})
	if err != nil {
	...
	foundNAT := false
	for _, c := range chains {
		if c.Table.Name == "flywall" && c.Name == "postrouting" {
			foundNAT = true
			break
		}
	}
