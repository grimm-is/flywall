...
type TableMetadata struct {
	Version    string // Flywall version (e.g., "1.2.3" or "dev")
	ApplyCount int    // Number of times rules have been applied
	ConfigHash string // First 8 chars of SHA256 of config
}

// metadataRegex parses the flywall metadata comment format:
// flywall:v<version>:c=<count>:h=<hash>
var metadataRegex = regexp.MustCompile(`flywall:v([^:]+):c=(\d+):h=([a-f0-9]+)`)
...
func GetTableMetadata(tableName, family string) *TableMetadata {
	// Run: nft list table inet flywall
...
func BuildMetadataComment(applyCount int, configHash string) string {
	...
	return fmt.Sprintf("flywall:v%s:c=%d:h=%s", version, applyCount, configHash)
}
