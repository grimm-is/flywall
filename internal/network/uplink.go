...
func (g *UplinkGroup) updateNewConnectionMark(srcNet string, oldMark, newMark RoutingMark) error {
	...
	if oldMark != 0 {
		_, _ = g.executor.RunCommand("nft", "delete", "rule", "inet", "flywall", "mark_prerouting",
			"ip", "saddr", srcNet,
			"ct", "state", "new",
	...
	_, err := g.executor.RunCommand("nft", "add", "rule", "inet", "flywall", "mark_prerouting",
		"ip", "saddr", srcNet,
		"ct", "state", "new",
...
func (g *UplinkGroup) Teardown() error {
	...
	for _, uplink := range g.Uplinks {
		...
		if uplink.LocalIP != "" {
			...
			DefaultCommandExecutor.RunCommand("nft", "delete", "rule", "inet", "flywall", "nat_postrouting",
	...
	// Flywall often flushes everything on apply?
...
func (g *UplinkGroup) setupConnmarkRestore(iface string) error {
	...
	// Fallback to shell command
	_, err := g.executor.RunCommand("nft", "insert", "rule", "inet", "flywall", "mark_prerouting",
		"iifname", iface,
...
func (g *UplinkGroup) setupSNAT(uplink *Uplink) error {
	...
	// Fallback to shell command
	_, err := g.executor.RunCommand("nft", "add", "rule", "inet", "flywall", "nat_postrouting",
		"meta", "mark", fmt.Sprintf("0x%x", uplink.Mark),
		"oifname", uplink.Interface,
		"snat", "to", uplink.LocalIP)
	return err
