...
// processPrefix is the global prefix used for log output (e.g., "FLYWALL", "FLYWALL-OLD", "FLYWALL-NEW")
var (
	processPrefix   = "FLYWALL"
	processPrefixMu sync.RWMutex
)

// SetPrefix sets the global log prefix (e.g., "FLYWALL-OLD" during upgrade)
...
func (h *ConsoleHandler) Handle(ctx context.Context, r slog.Record) error {
	...
	// Process Name and PID: flywall[12345]:
	// We use the configured prefix (usually "flywall") or os.Args[0] basename
	// User requested lower case processname[pid]:
	procName := strings.ToLower(GetPrefix())
	if procName == "" {
		procName = "flywall"
	}
	// Sanitize prefix if it was legacy "FLYWALL-OLD" -> "flywall"?
