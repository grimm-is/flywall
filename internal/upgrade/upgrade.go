...
func (m *Manager) InitiateUpgrade(ctx context.Context, newBinaryPath string, cfg *config.Config, configPath string) error {
	...
	// Override argv[0] so 'ps' shows reasonable name (e.g. /usr/sbin/flywall) not .../flywall_new
	...
	// When this parent process (flywall-old) exits, that pipe is closed.
	// If the child (flywall-new) tries to write to it (e.g. log output), it receives SIGPIPE and crashes.
