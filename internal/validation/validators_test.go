...
func TestValidatePath(t *testing.T) {
	allowedDirs := []string{"/etc/flywall", "/var/lib/flywall"}
	...
	tests := []struct {
		...
		{"allowed absolute", "/etc/flywall/firewall.hcl", false},
		{"allowed subdir", "/var/lib/flywall/state/db", false},

		// Sad paths
		{"empty", "", true},
		{"path traversal", "../../../etc/passwd", true},
		{"absolute not allowed", "/etc/passwd", true},
		{"null byte", "/etc/flywall/config\x00.hcl", true},
	}
