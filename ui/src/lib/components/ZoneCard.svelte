<script lang="ts">
  ...
  function navigateToLogs() {
    goto(`/observatory?filter=zone:${zone.name}`);
  }

  function scanSelf() {
    // TODO: Trigger self-scan API
    alert(`Scanning ${zone.name}...`);
  }

  function startCapture() {
    // Navigate to Observatory with capture parameter for this interface
    const iface = zone.interface || zone.name;
    goto(`/observatory?capture=${encodeURIComponent(iface)}`);
  }
    ...
    <div class="zone-title">
      ...
      {#if !isWan}
        ...
        {:else if learningMode === "tofu"}
          <span class="learning-badge" title="Trust on first use">🤝</span>
        {:else if learningMode === "approval"}
          <span class="learning-badge" title="Approval required">⏸️</span>
        {/if}
      {/if}
    </div>
    <button
  ...
  <div class="zone-grid">
    ...
    <div class="zone-section">
      ...
      <div class="section-content">
        {#if zone.ips && zone.ips.length > 0}
          ...
          {/each}
        {:else}
          <span class="ip-display font-mono">{zone.ip || "DHCP-Client"}</span>
        {/if}
      </div>
    </div>
    ...
    {#if isWan}
          ...
          {#if openPorts.length > 0}
            <div class="open-ports">
              ...
              {#if openPorts.length > 3}
                <span class="meta">+{openPorts.length - 3} more</span>
              {/if}
            </div>
          {/if}
        </div>
      ...
      <div class="zone-section">
        ...
        <div class="section-content">
          <span class="health-status good">● Up</span>
          <span class="meta">Latency: 12ms</span>
        </div>
      </div>
    {:else}
      <!-- LAN: Members -->
