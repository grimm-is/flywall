#!/bin/bash
#
# Proxmox LXC Installation Script for Flywall
#
...
VMID=""
CT_NAME="flywall" # Changed default name
STORAGE="local-lvm"
MEMORY=512
DISK=4
TEMPLATE=""
...
# Configure all interfaces to manual to allow Flywall full control
...
# Install Flywall binary
install_binary() {
    local vmid=$1
    log_info "Installing Flywall binary..."

    # Check locations:
    # 1. Current directory (from tarball)
    # 2. build/flywall (dev mode)
    if [ -f "./flywall" ]; then
        pct push "$vmid" ./flywall "${INSTALL_PREFIX:-/usr/sbin}/flywall"
        pct exec "$vmid" -- chmod +x "${INSTALL_PREFIX:-/usr/sbin}/flywall"
    elif [ -f "build/flywall" ]; then
        pct push "$vmid" build/flywall "${INSTALL_PREFIX:-/usr/sbin}/flywall"
        pct exec "$vmid" -- chmod +x "${INSTALL_PREFIX:-/usr/sbin}/flywall"
    else
        log_warn "Binary 'flywall' not found in . or build/. Skipping binary upload."
    fi
}

# Install Flywall services
install_flywall() {
    local vmid=$1
...
create_service() {
    local vmid=$1
    log_info "Creating Flywall Control Plane service..."
...
stop() {
    ebegin "Stopping Flywall"
    # Use the stop command which handles graceful shutdown
    '"${INSTALL_PREFIX:-/usr/sbin}"'/flywall stop
    eend $?
}
'
    pct exec "$vmid" -- sh -c "cat > /etc/init.d/flywall" <<EOF
$svc_content
EOF
    pct exec "$vmid" -- chmod +x /etc/init.d/flywall
    pct exec "$vmid" -- rc-update add flywall default
}
...
create_default_config() {
    ...
    # Start config header
    local config='# Flywall Configuration
...
'
    ...
    if $DRY_RUN; then
        log_info "Would create /etc/flywall/flywall.hcl"
        echo "$config"
        return
    fi
...
print_summary() {
    ...
    echo ""
    echo -e "${GREEN}════════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}  Flywall LXC Container Created Successfully${NC}"
    echo -e "${GREEN}════════════════════════════════════════════════════════════${NC}"
    echo ""
...
main() {
    echo ""
    echo -e "${BLUE}Proxmox LXC Flywall Installer${NC}"
    echo ""
    ...
    if ! $DRY_RUN; then
        ...
        # Reboot container to ensure clean state and start Flywall services
        log_info "Rebooting container to start Flywall..."
        pct stop "$vmid" 2>/dev/null || true
        # Wait for stop
        sleep 2
        pct start "$vmid"

        log_ok "Container started. Flywall should be running."
    fi
