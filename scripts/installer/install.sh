...
# Flywall Firewall Installer
# Usage: ./install.sh [target_disk]

echo "🔥 Flywall Firewall Installer"
echo "============================="
...
if [ ! -f "$SOURCE_DIR/flywall" ]; then
    echo "❌ Error: Flywall binaries not found in $SOURCE_DIR"
    exit 1
fi
...
echo "🔧 Installing Flywall binaries..."
mkdir -p "$MOUNTPOINT/usr/local/bin"
cp "$SOURCE_DIR/flywall" "$MOUNTPOINT/usr/local/bin/flywall"
...
cp "$SOURCE_DIR/flywall-ctl.init" "$MOUNTPOINT/etc/init.d/flywall-ctl"
cp "$SOURCE_DIR/flywall-api.init" "$MOUNTPOINT/etc/init.d/flywall-api"
chmod +x "$MOUNTPOINT/etc/init.d/flywall-ctl"
chmod +x "$MOUNTPOINT/etc/init.d/flywall-api"

echo "🔧 Configuring System..."
# Add firewall services to default runlevel
# We must chroot
chroot "$MOUNTPOINT" rc-update add flywall-ctl default
chroot "$MOUNTPOINT" rc-update add flywall-api default
