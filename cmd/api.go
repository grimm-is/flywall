...
type APIRuntimeConfig struct {
	ListenAddr     string
	DropUser       string
	NoSandbox      bool   // FLYWALL_NO_SANDBOX=1
	LocalUIDistDir string // FLYWALL_UI_DIST
	ForceTLS       bool   // FLYWALL_FORCE_TLS=1
	NoTLS          bool   // FLYWALL_NO_TLS=1 (force HTTP for tests)
}
...
func NewAPIRuntimeConfig(listenAddr, dropUser string) *APIRuntimeConfig {
	return &APIRuntimeConfig{
		...
		DropUser:       dropUser,
		NoSandbox:      os.Getenv("FLYWALL_NO_SANDBOX") == "1",
		LocalUIDistDir: os.Getenv("FLYWALL_UI_DIST"),
		ForceTLS:       os.Getenv("FLYWALL_FORCE_TLS") != "",
		NoTLS:          os.Getenv("FLYWALL_NO_TLS") == "1",
	}
}
...
func (e *alreadyRunningError) Error() string {
	return "flywall api is already running"
}
...
func (e *alreadyRunningError) LogAccessURLs() {
	...
	logging.Info("Flywall API is already running.")
...
func RunAPI(uiAssets embed.FS, listenAddr string, dropUser string) {
	...
	if err != nil {
		...
		logging.Info("Make sure 'flywall ctl' is running first.")
		os.Exit(1)
	}
	defer client.Close()
...
func setupTLS(rtCfg *APIRuntimeConfig, cfg *config.Config, isolated bool) (certFile, keyFile string, useTLS bool, isSelfSigned bool) {
	...
	if rtCfg.NoTLS {
		logging.Info("TLS disabled via FLYWALL_NO_TLS - running HTTP only")
		return "", "", false, false
	}
