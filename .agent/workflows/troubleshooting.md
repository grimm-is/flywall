...
Quick solutions for common issues when developing Flywall.
...
```
go: cannot find package "grimm.is/flywall/internal/foo"
```
...
```
/mnt/flywall/t/...: Permission denied
```
...
```bash
flywall check -v /path/to/config.hcl
```
...
ip link show flywall-api

# Restart ctl to recreate
kill $(pgrep -f "flywall ctl")
flywall ctl /etc/flywall/flywall.hcl &
```
...
1. Check logs: `flywall log -f` or `journalctl -u flywall`
2. Enable debug: `FLYWALL_DEBUG=1 flywall ctl ...`
