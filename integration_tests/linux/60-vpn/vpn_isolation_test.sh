...
if nft list table inet flywall 2>/dev/null | grep -qE "wg\*.*5353.*drop|DROP_MDNS"; then
    ok 0 "mDNS blocking rule for wg* interfaces present"
else
    diag "Looking for mDNS rules..."
    nft list table inet flywall 2>/dev/null | grep -i "5353\|mdns" || true
...
if nft list table inet flywall 2>/dev/null | grep -qE "tun\*.*5353.*drop|DROP_MDNS"; then
...
if nft list table inet flywall 2>/dev/null | grep -qE "udp dport 51820|wireguard"; then
...
if nft list table inet flywall 2>/dev/null | grep -qE "vpn|wg-test"; then
