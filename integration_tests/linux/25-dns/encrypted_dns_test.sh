...
# Verify DNS port 53 is open (Flywall binds it)
# Wait for port 53
...
if ss -tn dst :8853 | grep -q ESTAB; then
    ...
    if [ -s "$MOCK_LOG" ]; then
         ...
         log "WARN: No connection verification via ss/log. Checking Flywall logs."
         # Check Flywall logs for forwarding attempt?
    fi
fi
