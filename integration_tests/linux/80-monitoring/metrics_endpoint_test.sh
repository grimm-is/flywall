...
# 5. Verify key flywall metrics are present
EXPECTED_METRICS="flywall_|promhttp_|go_"
if grep -qE "$EXPECTED_METRICS" "$METRICS_OUTPUT"; then
    ok 0 "Flywall/Prometheus metrics present"
    diag "Sample metrics:"
    grep -E "^flywall_" "$METRICS_OUTPUT" | head -5
else
