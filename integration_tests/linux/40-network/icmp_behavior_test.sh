...
if nft list table inet flywall 2>/dev/null | grep -qE "icmp|l4proto icmp"; then
    ok 0 "ICMP rules present in firewall"
else
    diag "nftables rules:"
    nft list table inet flywall 2>/dev/null | head -30
...
if nft list table inet flywall 2>/dev/null | grep -qE "icmpv6|nd-neighbor"; then
