...
if ip netns exec client ping -c 1 -W 1 $TARGET_IP >/dev/null 2>&1; then
    ...
    nft list chain inet flywall forward 2>&1 | head -30 | sed 's/^/#   /' || echo "#   No forward chain"
    echo "# nftables prerouting (mangle):"
    nft list chain inet flywall mark_prerouting 2>&1 | head -30 | sed 's/^/#   /' || echo "#   No mark_prerouting chain"
    echo "# forward_vmap contents:"
    nft list map inet flywall forward_vmap 2>&1 | head -20 | sed 's/^/#   /' || echo "#   No forward_vmap"
    echo "# nftables output (mangle):"
    nft list chain inet flywall output 2>&1 | head -20 | sed 's/^/#   /' || echo "#   No output chain"
    echo "# policy_trusted_wan chain:"
    nft list chain inet flywall policy_trusted_wan 2>&1 | head -15 | sed 's/^/#   /' || echo "#   No policy_trusted_wan chain"
...
if ! nft list chain inet flywall mark_prerouting | grep -E "mark set 0x0*100"; then
    ...
    nft list chain inet flywall mark_prerouting
...
if nft list chain inet flywall mark_prerouting | grep -E "mark set 0x0*101"; then
