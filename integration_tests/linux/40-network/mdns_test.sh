...
# We use the main flywall binary with the hidden 'mcast' command we added
...
# Using the main flywall binary which we proven works in netns
...
PAYLOAD="FLYWALL_MDNS_TEST_PAYLOAD_$(date +%s)"
...
PAYLOAD_REV="FLYWALL_MDNS_REVERSE_$(date +%s)"
