...
if nft list table inet flywall 2>/dev/null | grep -qE "ct state.*established"; then
...
if nft list table inet flywall 2>/dev/null | grep -qE "ct state invalid.*drop"; then
    ok 0 "Invalid packet drop rule present"
else
    diag "Looking for invalid drop rule..."
    nft list table inet flywall 2>/dev/null | grep -i invalid
...
if nft list table inet flywall 2>/dev/null | grep -q "DROP_INVALID"; then
