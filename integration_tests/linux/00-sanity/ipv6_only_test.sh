...
RULESET=$(nft list table inet flywall 2>/dev/null || echo "")
...
if curl --version | grep -q IPv6; then
    if curl -s -o /dev/null -w "%{http_code}" "http://[::1]:$TEST_API_PORT/api/status" 2>/dev/null | grep -q "200\|401"; then
