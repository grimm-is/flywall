...
if nft list table inet flywall 2>/dev/null | grep -qE "saddr 10\.0\.0\.0/8|saddr 192\.168\.0\.0/16|saddr 172\.16\.0\.0/12"; then
    ...
    nft list table inet flywall 2>/dev/null | head -40
...
nft list table inet flywall 2>/dev/null | grep -q "drop"
ok $? "Drop rules present in protection chain"

# Test 6: Check for bogon filtering rules
diag "Checking bogon filtering rules..."
if nft list table inet flywall 2>/dev/null | grep -qE "saddr 0\.0\.0\.0/8|saddr 127\.0\.0\.0/8"; then
...
# Check if any flywall-related logs appear
if dmesg | grep -qE "flywall|PROTECTION|ACCEPT|DROP"; then
