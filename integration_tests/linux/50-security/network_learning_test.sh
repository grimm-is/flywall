...
if nft list chain inet flywall forward 2>/dev/null | grep -qE "tcp dport 443.*log"; then
    ok 0 "TLS SNI logging rule present"
else
    diag "Looking for TLS logging rule..."
    nft list chain inet flywall forward 2>/dev/null | grep -i "443" || true
...
if nft list chain inet flywall forward 2>/dev/null | grep -qE "log group 100|TLS_SNI"; then
...
if nft list chain inet flywall forward 2>/dev/null | grep -qE "443.*limit rate"; then
...
if nft list chain inet flywall forward 2>/dev/null | grep -qE "443.*ct state established"; then
