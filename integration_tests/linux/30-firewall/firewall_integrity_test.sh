...
nft flush chain inet flywall input

# Poll for Revert
echo "Polling for integrity monitor..."
count=0
while ! nft list chain inet flywall input 2>/dev/null | grep -q "ct state"; do
    dilated_sleep 0.5
    count=$((count + 1))
...
if ! nft list chain inet flywall input | grep -q "ct state"; then
