...
# Test 1: Ensure clean state by deleting any existing flywall table
diag "Checking initial state..."
# Clean up any existing flywall tables from previous test runs
nft delete table inet flywall 2>/dev/null || true
nft delete table ip nat 2>/dev/null || true
# Verify cleanup worked
echo "$(nft list ruleset 2>/dev/null)" | grep -q "table inet flywall"
[ $? -ne 0 ]
ok $? "Clean state - no flywall table exists"
...
cleanup() {
    rm -f "$TEST_CONFIG"
    # Clean up nftables
    nft delete table inet flywall 2>/dev/null
...
# Test 3: flywall table now exists
diag "Checking flywall table was created..."
echo "$AFTER_RULES" | grep -q "table inet flywall"
ok $? "flywall table created"

# Test 4: Forward chain exists
diag "Checking forward chain..."
nft list chain inet flywall forward >/dev/null 2>&1
ok $? "forward chain exists"

# Test 5: Input chain exists
diag "Checking input chain..."
nft list chain inet flywall input >/dev/null 2>&1
ok $? "input chain exists"

# Test 6: Output chain exists
diag "Checking output chain..."
nft list chain inet flywall output >/dev/null 2>&1
ok $? "output chain exists"

# Test 7: Check for established/related rule
diag "Checking established/related rule..."
nft list chain inet flywall forward | grep -E "ct state established,related.*accept"
...
nft list chain inet flywall policy_wan_lan | grep -E "counter.*drop"
...
nft list chain inet flywall policy_lan_wan | grep -E "counter.*accept"
...
RULE_COUNT=$(nft list table inet flywall | grep -c "^[[:space:]]*\(accept\|drop\|reject\|counter\)" || echo 0)
...
# NAT is in a separate 'ip nat' table, not 'inet flywall'
...
TABLE_COUNT=$(nft list tables | grep -c "table inet flywall$" || echo 0)
    if [ "$TABLE_COUNT" -eq 1 ]; then
        ok 0 "Exactly one flywall table exists"
    else
        ok 1 "Exactly one flywall table exists (Count: $TABLE_COUNT)"
...
nft list chain inet flywall forward | grep -q "type filter"
...
if [ $failed_count -eq 0 ]; then
    ...
    nft list table inet flywall 2>/dev/null | head -50
