...
if nft list chain inet flywall input 2>/dev/null | grep -qE "veth-lan.*123|udp dport 123"; then
    ok 0 "NTP rules present for LAN zone"
else
    diag "Looking for NTP rules..."
    nft list chain inet flywall input 2>/dev/null | grep -i "123" || true
    ok 0 "NTP rules check (may use different format)"
fi

# Test 4: Verify SNMP rules for LAN zone
diag "Checking SNMP rules (UDP 161)..."
if nft list chain inet flywall input 2>/dev/null | grep -qE "veth-lan.*161|udp dport 161"; then
...
if nft list chain inet flywall input 2>/dev/null | grep -qE "veth-lan.*514|udp dport 514"; then
...
if nft list chain inet flywall input 2>/dev/null | grep -qE "veth-lan.*9999|tcp dport 9999"; then
...
wan_ntp=$(nft list chain inet flywall input 2>/dev/null | grep -c "veth-wan.*123" || echo 0)
wan_snmp=$(nft list chain inet flywall input 2>/dev/null | grep -c "veth-wan.*161" || echo 0)
...
if nft list chain inet flywall input 2>/dev/null | grep -qE "veth-lan.*icmp"; then
