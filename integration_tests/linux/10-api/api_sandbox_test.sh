...
if nft list chain inet flywall forward 2>/dev/null | grep -qE "169.254.255.2.*accept"; then
...
if nft list chain inet flywall output 2>/dev/null | grep -qE "169.254.255.2.*accept"; then
