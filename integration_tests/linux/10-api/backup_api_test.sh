...
HTTP_CODE=$(curl -s -m 10 --retry 3 --retry-delay 1 --retry-all-errors \
    -o /tmp/backups_$$.json -w "%{http_code}" \
    "http://127.0.0.1:$TEST_API_PORT/api/backups")
...
RESPONSE=$(curl -s -m 10 --retry 2 --retry-delay 1 --retry-all-errors \
    -X POST "http://127.0.0.1:$TEST_API_PORT/api/backups/create" \
...
if echo "$RESPONSE" | grep -qE '"version"|"success"'; then
    ...
    ok 1 "Backup creation returned: $RESPONSE" severity fail
fi

# Test 4: GET backup content (may fail if no backups exist)
diag "Test 4: GET backup content"
BACKUP_VERSION=$(curl -s -m 5 "http://127.0.0.1:$TEST_API_PORT/api/backups" | jq -r '.backups[0].version // empty')
if [ -n "$BACKUP_VERSION" ]; then
    HTTP_CODE=$(curl -s -m 10 -o /tmp/backup_content_$$.json -w "%{http_code}" \
        "http://127.0.0.1:$TEST_API_PORT/api/backups/content?version=$BACKUP_VERSION")
    if [ "$HTTP_CODE" -eq 200 ]; then
    ...
    ok 1 "GET backup content skipped (no backups)" severity fail
fi
...
HTTP_CODE=$(curl -s -m 10 -o /dev/null -w "%{http_code}" -X POST \
    "http://127.0.0.1:$TEST_API_PORT/api/backups/restore" \
    -H "Content-Type: application/json" \
    -H "X-API-Key: test-bypass" \
    -d '{"version": 999999}')
...
if [ "$HTTP_CODE" -eq 404 ] || [ "$HTTP_CODE" -eq 400 ]; then
    ...
    ok 1 "Invalid version returns $HTTP_CODE" severity fail
fi
