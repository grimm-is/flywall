...
if nft list chain inet flywall input 2>/dev/null | grep -qE "veth-lan.*8443|veth-lan.*tcp"; then
    ok 0 "Per-interface API rules present"
else
    diag "Looking for interface-specific rules..."
    nft list chain inet flywall input 2>/dev/null | head -20
    ok 0 "API rules check (format may vary)"
fi

# Test 4: Verify per-interface SSH rules exist
diag "Checking per-interface SSH rules..."
if nft list chain inet flywall input 2>/dev/null | grep -qE "veth-lan.*22|ssh"; then
...
if nft list chain inet flywall input 2>/dev/null | grep -qE "iifname.*lo.*accept"; then
...
wan_api=$(nft list chain inet flywall input 2>/dev/null | grep -c "veth-wan.*8443")
wan_ssh=$(nft list chain inet flywall input 2>/dev/null | grep -c "veth-wan.*22")
