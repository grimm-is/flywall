...
if grep -q "ENTERING SAFE MODE" "$CTL_LOG"; then
    ...
    exit 0
elif [ -f "$LOG_DIR/flywall.log" ] && grep -q "ENTERING SAFE MODE" "$LOG_DIR/flywall.log"; then
    ...
    cat "$CTL_LOG"
    if [ -f "$LOG_DIR/flywall.log" ]; then
        echo "Checking $LOG_DIR/flywall.log:"
        cat "$LOG_DIR/flywall.log"
    fi
    exit 1
fi
