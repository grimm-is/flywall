...
if $APP_BIN check "$CONFIG_VALID" >/dev/null 2>&1; then
    ok 0 "flywall check passed for valid config"
else
    ok 1 "flywall check failed for valid config"
fi

# Scenario 2.2: Invalid config should fail
if ! $APP_BIN check "$CONFIG_INVALID" > /dev/null 2>&1; then
    ok 0 "flywall check failed (correctly) for invalid config"
else
    ok 1 "flywall check passed (incorrectly) for invalid config"
fi
...
if [ $EXIT_CODE -eq 0 ]; then
    ok 0 "flywall apply --dry-run exit code 0"
else
...
if ! $APP_BIN apply -n "$CONFIG_INVALID" > /dev/null 2>&1; then
    ok 0 "flywall apply -n failed (correctly) for invalid config"
else
    ok 1 "flywall apply -n passed (incorrectly) for invalid config"
fi
...
if [ $DIFF_EXIT -eq 0 ]; then
    if echo "$DIFF_OUTPUT" | grep -q "No changes detected"; then
        ok 0 "flywall diff detected no changes"
    else
        ok 0 "flywall diff exit 0 confirms no changes"
    fi
else
    ok 0 "flywall diff ran successfully (exit code $DIFF_EXIT indicates diffs)"
    echo "# Diff Output:"
