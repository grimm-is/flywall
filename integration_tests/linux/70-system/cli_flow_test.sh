...
if $FLYWALL start --config "$CONFIG_FILE"; then
    ...
    echo "not ok 1 - flywall start command failed"
    exit 1
fi
...
if echo "$API_OUTPUT" | grep -q "Flywall API is already running\|Waiting for previous API instance\|API server is already running\|address already in use"; then
    ...
    echo "not ok 5 - flywall test-api failed to detect running instance"
    echo "# Output:"
...
if $FLYWALL stop; then
    echo "ok 6 - flywall stop command succeeded"
else
    echo "not ok 6 - flywall stop command failed"
    exit 1
fi
