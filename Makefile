...
# On macOS: Builds Client (flywall)
# On Linux: Builds Server (flywall)
build: ui $(BINARY)
...
$(BUILD_DIR)/toolbox: $(TOOLBOX_HOST)
	...
	@ln -sf $(notdir $(TOOLBOX_HOST)) build/flywall-orca
...
build-builder:
	@echo "$(BLUE)Building flywall-builder...$(NC)"
	@go build -o $(BUILD_DIR)/flywall-builder ./cmd/flywall-builder

build-iso: build-builder build-go
	@echo "$(BLUE)Building installer ISO...$(NC)"
	@$(BUILD_DIR)/flywall-builder iso
...
install-lxc: server
	...
	@echo "  ssh root@proxmox './install-proxmox.sh --name flywall'"
	@echo ""
	@echo "$(YELLOW)Commands:$(NC)"
	@echo "  ./install-proxmox.sh --name flywall --wan vmbr0 --lan vmbr1"
...
dist-proxmox:
	...
	@# Build linux binary
	@$(MAKE) server
	@cp $(BINARY_TARGET) $(BUILD_DIR)/proxmox-dist/flywall
	...
	@# Create tarball
	@tar -czf $(BUILD_DIR)/flywall-proxmox.tar.gz -C $(BUILD_DIR)/proxmox-dist .
	@rm -rf $(BUILD_DIR)/proxmox-dist
	@echo "$(GREEN)Bundle created: $(BUILD_DIR)/flywall-proxmox.tar.gz$(NC)"
	@echo "To deploy: scp $(BUILD_DIR)/flywall-proxmox.tar.gz root@proxmox:~/"
	@echo ""

# Create self-extracting installer
dist-proxmox-installer: dist-proxmox
	@echo "$(BLUE)Building Self-Extracting Installer...$(NC)"
	@cat scripts/build/self-extract.sh $(BUILD_DIR)/flywall-proxmox.tar.gz > $(BUILD_DIR)/install-flywall-proxmox.sh
	@chmod +x $(BUILD_DIR)/install-flywall-proxmox.sh
	@echo "$(GREEN)Installer created: $(BUILD_DIR)/install-flywall-proxmox.sh$(NC)"
	@echo "To deploy: scp $(BUILD_DIR)/install-flywall-proxmox.sh root@proxmox:~/"
	@echo "           ssh root@proxmox './install-flywall-proxmox.sh --name firewall'"
	@echo ""
