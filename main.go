...
func main() {
	...
	if os.Getenv("FLYWALL_UPGRADE_STANDBY") == "1" {
		// Prevent infinite recursion in children (like flywall api) by cleaning the env
	...
	case "apply":
		...
		// If the user wants "apply changes to running daemon", they should use `flywall config apply` via API?
	...
	case "_api-server":
		...
		// This is not a public command - use `flywall start` instead
	...
	case "test-api":
		...
		// Usage: flywall test-api [-listen :8080]
		...
		os.Setenv("FLYWALL_NO_SANDBOX", "1")
		if *noTLS {
			os.Setenv("FLYWALL_NO_TLS", "1")
		}
	...
	case "console":
		...
		remote := consoleFlags.String("remote", "", "Remote Flywall API URL")
	...
	case "reload":
		...
		if len(reloadFlags.Args()) > 0 {
			// e.g. flywall reload myconfig.hcl
	...
	case "help", "-h", "--help":
		// Help command
		if len(os.Args) > 2 {
			// Command-specific help: flywall help <command>
			switch os.Args[2] {
			case "api", "apikey":
...
Examples:
  ...
  %s check -v /etc/flywall/flywall.hcl
